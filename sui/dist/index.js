...
var SUI_BASE_NUM = new import_bignumber.default(1e9);
...
var Sui = class extends Blockchain {
  ...
  initializeNetwork(network, url) {
    ...
    this.client = new import_client.SuiClient({
  ...
  async getBalanceByAddress(address) {
    ...
    try {
      ...
      return new import_bignumber2.default(balance.totalBalance);
    } catch (error) {
  ...
  async stake(amount) {
    ...
    try {
      ...
      tx.moveCall({
        ...
        arguments: [
          tx.sharedObjectRef({
            objectId: import_utils2.SUI_SYSTEM_STATE_OBJECT_ID,
  ...
  async unstake(stakedSuiId) {
    try {
      ...
      tx.moveCall({
        ...
        arguments: [
          tx.object(import_utils2.SUI_SYSTEM_STATE_OBJECT_ID),
   ...
   */
  isAddress(address) {
    return (0, import_utils2.isValidSuiAddress)(address);
  }
};
