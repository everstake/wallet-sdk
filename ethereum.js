...
async function unstake(address, amount, isAllowedInterchange, source = '0') {
    try {
        const amountWei = await web3.utils.toWei(amount.toString(), 'ether');
        ...
        if (balance >= +amount) {
            ...
            return {
                ...
                'to': ADDRESS_CONTRACT_POOL,
                'gas': baseGas,
                'data': contract_pool.methods.unstake(amountWei, isAllowedInterchange, source).encodeABI()
            };
        } else {
